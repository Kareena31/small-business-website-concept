...
async function loadSite() {
  ...
  if (!siteConfig.demo_mode) {
    ...
    if (!sessionResponse.ok) throw new Error("The request form is temporarily unavailable. Please call the restaurant.");
...
document.querySelector("#sample-button").addEventListener("click", () => {
  ...
  form.elements.visit_details.value = "Sample group of 4";
  form.elements.email.value = "sample@example.com";
  form.elements.phone.value = "";
  form.elements.service.value = "Visit or group question";
  form.elements.message.value = "This is a fictional sample inquiry. I would like to ask about visiting with a small group next week.";
...
form.addEventListener("submit", async event => {
  ...
  if (!sessionInfo) { showStatus("The form is temporarily unavailable. Please call the restaurant.", true); return; }
  const data = Object.fromEntries(new FormData(form));
  data.consent = form.elements.consent.checked;
  if (!data.email.trim() && !data.phone.trim()) { showStatus("Add your email or phone number so the restaurant can reply.", true); return; }
  ...
  try {
    ...
    form.reset();
  } catch(error) { showStatus(error.message || "Connection failed. Please call the restaurant.", true); }
