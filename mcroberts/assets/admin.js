...
function render() {
  ...
  document.querySelector("#stat-progress").textContent = inquiries.filter(item => ["contacted","scheduled"].includes(item.status)).length;
  ...
  inquiries.forEach(item => {
    ...
    content.append(element("p", "#" + item.id + " · " + new Date(item.created_at * 1000).toLocaleString() + (item.vehicle ? " · " + item.vehicle : ""), "meta"));
    ...
    select.addEventListener("change", async () => {
      ...
      try {
        ...
        render();
        notify(config.demo_mode ? "Preview updated. No customer was contacted and nothing was saved." : "Request status saved. Contact the customer separately to confirm any appointment.");
...
async function initialize() {
  ...
  if (config.demo_mode) {
    ...
    inquiries = [
      {id:103,created_at:1789198200,name:"Sample Customer A",email:"sample-a@example.com",phone:"",vehicle:"2019 Sample Vehicle",service:"Repair inquiry",message:"Fictional preview: I hear a new noise when braking. Could we discuss a visit?",status:"new"},
      {id:102,created_at:1789192800,name:"Sample Customer B",email:"sample-b@example.com",phone:"",vehicle:"2021 Sample Vehicle",service:"Towing or transport question",message:"Fictional preview: I would like to discuss a planned vehicle transport next week.",status:"contacted"},
      {id:101,created_at:1789117200,name:"Sample Customer C",email:"sample-c@example.com",phone:"",vehicle:"",service:"Something else",message:"Fictional preview: Thank you for calling. We agreed to a visit on Tuesday.",status:"scheduled"}
    ];
    await showDashboard();
  } else {
